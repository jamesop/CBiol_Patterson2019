function [cellTotPar,matCellGrowth]=cellTimerTotDivSim(growthRate,numCells,simTime,numNoise,meanTime,meanDivLen)
...
for i=1:simTime;
    ...
    matDivSize(currCellEnd==1)=normrnd(meanTime,numNoise,sum(currCellEnd==1),1);
    cDivSize=matDivSize(matGrow);
    %Include if using a timer
    currpDivEv=(valEnds>cDivSize) & (valEnds>threshTime);
